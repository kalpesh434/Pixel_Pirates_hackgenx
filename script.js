...
// API base URL - points to our MongoDB Express server
const API_BASE_URL = 'http://localhost:3000/api';

// Check MongoDB API connection on startup
function checkMongoDBConnection() {
    ...
    fetch(`${API_BASE_URL}/health`)
        ...
        .then(data => {
            ...
            if (data.status === 'ok') {
                statusElement.textContent = 'Connected to MongoDB successfully!';
                statusElement.style.color = '#4CAF50';
                console.log('Successfully connected to MongoDB API');
            }
        })
        .catch(error => {
            console.error('Failed to connect to MongoDB API:', error);
            statusElement.textContent = 'Failed to connect to MongoDB. Using local data.';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show loader for 2.5 seconds to ensure animation is seen
    setTimeout(() => {
        // Fade out loader
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                document.getElementById('login-page').style.transition = 'opacity 0.5s';
            }, 50);
        }, 500);
    }, 2500);
...
// Function to show and reset the government dashboard
function showGovernmentDashboard() {
    ...
    dashboard.style.display = 'block'; // Ensure it's displayed
    
    // Make sure public dashboard is hidden
    ...
    console.log("Government dashboard initialized");
}

// Modify the login function to use our new dashboard function
function handleLogin(e) {
    e.preventDefault();
    
    // Get the user type
    const isPublicUser = document.querySelector('.user-type-card:nth-child(1)').classList.contains('active');
    console.log("Selected user type:", isPublicUser ? "public" : "government");
    ...
    // For testing, allow government login with hardcoded credentials
    if ((userData.email === 'admin@gov.in' || userData.mobile === '9876543210') && 
        (userData.password === 'admin123')) {
        console.log("Using hardcoded government credentials for testing");
    }
    ...
    .then(data => {
        if (data.success) {
            ...
            setTimeout(() => {
                ...
                // Show appropriate dashboard based on user role (not selected type)
                const userRole = data.user.role || 'public';
                console.log("User role from database:", userRole);
                
                if (userRole === 'government') {
                    console.log("Showing government dashboard");
                    showGovernmentDashboard();
                } else {
                    console.log("Showing public dashboard");
...
function loadBudgetData() {
    fetch(`${API_BASE_URL}/budget/current`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            // Update UI with the budget data
            const formattedAmount = new Intl.NumberFormat('en-IN').format(data.totalAmount);
            document.querySelector('.budget-amount').textContent = `₹${formattedAmount} Crores`;
            document.querySelector('.budget-year').textContent = `Fiscal Year ${data.fiscalYear}`;
            
            // Map data for sector cards
...
function submitFeedback() {
    ...
    const satisfaction = parseInt(document.getElementById('feedback-satisfaction').value);
    ...
    fetch(`${API_BASE_URL}/feedback`, {
        ...
        body: JSON.stringify({ topic, text, satisfaction })
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        if (data.success) {
            // Show success message
            document.getElementById('public-feedback-confirmation').classList.remove('hidden');
            
            // Clear form
            document.getElementById('feedback-text').value = '';
            document.getElementById('feedback-satisfaction').value = '5';
            
            // Load updated feedback analysis
            loadFeedbackAnalysis();
            
            // Hide confirmation after 5 seconds
            setTimeout(() => {
                document.getElementById('public-feedback-confirmation').classList.add('hidden');
            }, 5000);
        } else {
...
// Load feedback analysis from API
function loadFeedbackAnalysis() {
    fetch(`${API_BASE_URL}/feedback/analysis`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            document.getElementById('public-total-feedback-count').textContent = data.totalFeedback;
            document.getElementById('public-overall-satisfaction').textContent = data.overallSatisfaction;
            document.getElementById('public-top-concern').textContent = formatTopic(data.topConcern);
            
            // If available in the government dashboard
            if (document.getElementById('total-feedback-count')) {
                document.getElementById('total-feedback-count').textContent = data.totalFeedback;
                document.getElementById('overall-satisfaction').textContent = data.overallSatisfaction;
                document.getElementById('top-concern').textContent = formatTopic(data.topConcern);
            }
            
            // Show feedback results
            document.getElementById('public-feedback-results').classList.remove('hidden');
            
            // If there are themes, update the table
            if (data.feedbackThemes && data.feedbackThemes.length > 0) {
                updateFeedbackTable(data.feedbackThemes);
            }
        })
        .catch(error => {
